...
const FollowUpManager: React.FC = () => {
      ...
      try {
        ...
        const mappedFollowUps: FollowUp[] = data.map((apiLead) => {
          ...
          return {
            ...
            completedDate: currentStatus === 'completed' ? apiLead.nextFollowUpDate || new Date().toISOString().split('T')[0] : undefined,
  ...
  const handleEditFollowUp = async () => {
    ...
    try {
      ...
      await axios.patch(`/api/leads/${editFollowUp.leadId}`, apiPayload);
  ...
  const stats = useMemo(() => {
    const todayCount = followUps.filter(f => isToday(f.scheduledDate) && f.status === 'pending').length;
    const overdueCount = followUps.filter(f => f.status === 'overdue').length;
    const pendingCount = followUps.filter(f => f.status === 'pending').length;
    const completedThisWeekCount = followUps.filter(f => {
    ...
    return { today: todayCount, overdue: overdueCount, pending: pendingCount, completedThisWeek: completedThisWeekCount };
  }, [followUps]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        ...
        {/* 2️⃣ Search + Filters + Add Follow-up */}
        <div className="flex flex-col sm:flex-row gap-4 justify-between">
          ...
          <div className="flex gap-4 flex-wrap">
            ...
            <Select value={filterPriority} onValueChange={setFilterPriority}>
              ...
              <SelectContent>
                ...
                {priorities.map((priority) => (<SelectItem key={priority} value={priority}>{priority.charAt(0).toUpperCase() + priority.slice(1)}</SelectItem>))}
              </SelectContent>
            </Select>
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
            ...
            <DialogContent className="max-w-2xl">{/* Add Dialog Form here */}</DialogContent>
          </Dialog>
        </div>

        {/* 3️⃣ Follow-ups Table */}
        <Card>
                  ...
                  {filteredFollowUps.map(followUp => (
                    <tr key={followUp.id} className={`border-b hover:bg-gray-50 ${isToday(followUp.scheduledDate) && followUp.status === 'pending' ? 'bg-blue-50' : ''}`}>
                      <td className="p-4">
                        ...
                        {isToday(followUp.scheduledDate) && followUp.status === 'pending' && <Badge variant="outline" className="text-xs mt-1 bg-blue-100 text-blue-800">Due Today</Badge>}
                      </td>
                      <td className="p-4">
        ...
        {/* 4️⃣ Dialogs */}
        {/* ... all dialogs remain exactly as in your original snippet ... */}
      </div>
      );
...
const FollowUpManagerWithErrorBoundary: React.FC = () => (
  <ErrorBoundary><FollowUpManager /></ErrorBoundary>
);
