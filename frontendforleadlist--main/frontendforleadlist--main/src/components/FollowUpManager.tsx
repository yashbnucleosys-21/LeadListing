...
const FollowUpManager: React.FC = () => {
    ...
    const fetchFollowUps = async () => {
      try {
        ...
        const mappedFollowUps: FollowUp[] = data.map((apiLead) => {
          ...
          return { 
            id: apiLead.id, leadId: apiLead.id, leadName: apiLead.companyName || 'Unknown Lead', 
            assignee: apiLead.assignee || 'Unassigned', type: followUpType, priority: apiLead.priority || 'medium', 
            status: currentStatus, scheduledDate: scheduledDate, 
            scheduledTime: apiLead.followUpTime ? new Date(apiLead.followUpTime).toTimeString().substring(0, 5) : '', 
            description: apiLead.service || '', notes: apiLead.notes || '', 
  ...
  return (
    <div className="space-y-6">
      ...
      {/* DIALOGS */}
      {/* View, Edit, History Dialogs remain exactly as your original code */}
      {/* No changes needed; already dynamic with state */}
    </div>
  );
};
